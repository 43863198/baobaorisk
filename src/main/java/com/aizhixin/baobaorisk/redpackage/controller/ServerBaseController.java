...
public class ServerBaseController {
    ...
    public WxPrePayVO order(@ApiParam(value = "openId", required = true) @PathVariable String openId,
                            @ApiParam(value = "红包总金额(大于0，小于200)", required = true) @RequestParam (value = "totalFee") Double totalFee,
                            @ApiParam(value = "红包数量(大于0，小于100)", required = true) @RequestParam (value = "num") Integer num) {
        return payService.createOrder(openId, totalFee, num);
    }
}
